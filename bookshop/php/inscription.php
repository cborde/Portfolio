...
$err = isset($_POST['btnSInscrire']) ? bgl_inscription() : array(); 

bg_html_debut('BookShop | Inscription', '../styles/bookshop.css');

bg_bookshop_enseigne_entete(false,'../');
	...
	echo 	
		'<form method="post" action="inscription.php">',
			...
			'<table>',
				...
				bg_form_ligne('Date de naissance :', bg_form_date('naiss', NB_ANNEES_DATE_NAISSANCE, $naiss_j, $naiss_m, $naiss_a)),
				'<tr><td colspan="2" style="padding-top: 10px;" class="centered">', bg_form_input(bg_Z_SUBMIT,'btnSInscrire','Je m\'inscris !'), '</td></tr>',
			'</table>',
		'</form>';
}	
 ...
 *
 */
function bgl_control_piratage(){
    $nb = count($_POST);
    if ($nb == 2){
        (! isset($_POST['btnInscription']) || $_POST['btnInscription'] != 'S\'inscrire') && bg_exit_session();
        (! isset($_POST['source'])) && bg_exit_session();
        (strip_tags($_POST['source']) != $_POST['source']) && bg_exit_session();
        return;     // => ok, pas de problème détecté
    }
    if ($nb == 9){
        (! isset($_POST['btnSInscrire']) || $_POST['btnSInscrire'] != 'Je m\'inscris !') && bg_exit_session();
        (! isset($_POST['source'])) && bg_exit_session();
        (strip_tags($_POST['source']) != $_POST['source']) && bg_exit_session();
        (! isset($_POST['email'])) && bg_exit_session();
        (! isset($_POST['pass1'])) && bg_exit_session();
        (! isset($_POST['pass2'])) && bg_exit_session();
        (! isset($_POST['nomprenom'])) && bg_exit_session();
        (! isset($_POST['naiss_j'])) && bg_exit_session();
        (! isset($_POST['naiss_m'])) && bg_exit_session();
        (! isset($_POST['naiss_a'])) && bg_exit_session();
        (!est_entier($_POST['naiss_a']) || !est_entier($_POST['naiss_m']) || !est_entier($_POST['naiss_j'])) && bg_exit_session();
        $aa = date('Y');
        ($_POST['naiss_j'] < 1 || $_POST['naiss_j'] > 31 || $_POST['naiss_m'] < 1 || $_POST['naiss_m'] > 12 ||
        $_POST['naiss_a'] > $aa || $_POST['naiss_a'] <= $aa - NB_ANNEES_DATE_NAISSANCE) && bg_exit_session();
...
function bgl_inscription() {
    ...
    $invalid = bg_INVALID_STRING;
    $code_postal = bg_INVALID_CODE_POSTAL;
