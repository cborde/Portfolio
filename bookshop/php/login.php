...
// si $_POST non vide
$err = ($_POST) ? bgl_traitement_connexion() : 0;

bg_html_debut('BookShop | Connexion', '../styles/bookshop.css');
	...
	echo
        ...
        '<div id="bcInscription">', 
            '<form action="login.php" method="post" class="bcFormulaireBoite">',			
                ...
                '<table>', 
                    bg_form_ligne('Email :', bg_form_input(bg_Z_TEXT,'email','',20)),
                    bg_form_ligne('Mot de passe :', bg_form_input(bg_Z_PASSWORD,'password','',20)),
                '</table>',
                bg_form_input(bg_Z_SUBMIT,'btnConnexion', 'Se connecter'),
            '</form>',
            ...
            '<form action="inscription.php" method="post" class="bcFormulaireBoite">',			
                ...
                bg_form_input(bg_Z_SUBMIT,'btnInscription', 'S\'inscrire'),
            '</form>', 
        '</div>'; 
}
 ...
 */
function bgl_traitement_connexion() {

	// connexion à la base de données	
	$bd = bg_bd_connect();

	// sanitization des données postées
	$email = bg_bd_protect($bd, $_POST['email']);
	$password = bg_bd_protect($bd, md5($_POST['password']));
